// Generated by /webbuild make_config.py — per-venue tokens (see gravura/TEMPLATE.md)

export const site = {
  canonicalUrl: "https://mitev22.github.io/webbuild-plovdiv-wedding-venues/sites/ermitazh",
  metaTitle: "Зала „Ермитаж“ | Тържества в Пловдив",
  metaDescription: "Зала за тържества на бул. „България“ 97 в Пловдив. Запитване за дата на 089 449 9866.",
  specDemo: true,
} as const;

export const brand = {
  nameBg: "Зала „Ермитаж“",
  nameEn: "Ermitazh Hall",
  town: "Пловдив",
  since: "",
  phone: "089 449 9866",
  phoneHref: "tel:+359894499866",
  email: "",
  logoSrc: "",
  logoInNav: false,
  address: "бул. „България“ 97, Пловдив",
  mapEmbedSrc: "",
  instagram: "",
  facebook: "",
} as const;

export const palette = {
  note: "Зала „Ермитаж“: no brand materials published — template palette kept as documented.",
} as const;

export const images = {
  heroFilm: "",
  wallGarden: "",
  wallHall: "",
  wallTerrace: "",
  tableBand: "",
  heroTone: "",
  ogImage: "/atmosphere/og-card.jpg",
} as const;

export const factsDict = {
  bg: {
    capacityRows: [
      {
        space: "Залата",
        note: "бул. „България“ 97",
        value: "капацитет по запитване",
      },
    ],
    timetable: [
      {
        what: "Посрещане на гостите в залата",
        when: "начало",
      },
      {
        what: "Вечеря",
        when: "вечерта",
      },
      {
        what: "Танците продължават",
        when: "до късно",
      },
    ],
    packagesNote: "Зала „Ермитаж“ не публикува куверт. Опишете деня си във формата и ще получите лична оферта.",
  },
  en: {
    capacityRows: [
      {
        space: "The hall",
        note: "97 Bulgaria Blvd",
        value: "capacity on request",
      },
    ],
    timetable: [
      {
        what: "Guests are received in the hall",
        when: "start",
      },
      {
        what: "Dinner",
        when: "evening",
      },
      {
        what: "The dancing continues",
        when: "till late",
      },
    ],
    packagesNote: "Ermitazh Hall publishes no per-head price. Describe your day in the form for a personal offer.",
  },
} as const;

export const copyDict = {
  bg: {
    heroTitle: "Зала за деня на бул. „България“.",
    heroUnder: "На номер 97, в северната част на Пловдив.",
    heroCaption: "Час преди гостите.",
    heroCaptionNote: "Атмосферна визуализация",
    ctaEnquiry: "Запитване за дата",
    ctaVisit: "Заповядайте на оглед",
    wallTitle: "Залата на бул. „България“.",
    duetTitle: "Вечерта минава на едно място.",
    inviteLine: "Заповядайте на оглед. Огледът трае час и не задължава с нищо.",
    enquiryTitle: "Запитване за дата",
    enquiryLead: "Опишете деня си: сезон, брой гости, ориентировъчен бюджет. Отговаряме лично на всяко запитване.",
    atmosphereNote: "атмосферни визуализации, предстои добавяне на снимки",
  },
  en: {
    heroTitle: "A hall for the day on Bulgaria Boulevard.",
    heroUnder: "At number 97, in the northern part of Plovdiv.",
    heroCaption: "An hour before the guests.",
    heroCaptionNote: "Atmospheric visualisation",
    ctaEnquiry: "Enquire about a date",
    ctaVisit: "Come for a viewing",
    wallTitle: "The hall on Bulgaria Boulevard.",
    duetTitle: "The evening, from start to finish.",
    inviteLine: "Come for a viewing. It commits you to nothing.",
    enquiryTitle: "Enquire about a date",
    enquiryLead: "Tell us about your day: season, guest count, approximate budget. Every enquiry gets a personal reply.",
    atmosphereNote: "atmospheric visualisations, venue photography to be added",
  },
} as const;
